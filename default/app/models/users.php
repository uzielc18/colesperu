...
class Users extends ActiveRecord {
    ...
    public function obtener_usuarios_con_num_acciones($pagina = 1) {
        $cols = "users.*,aclroles.rol,COUNT(aclauditorias.id) as num_acciones";
        $join = "INNER JOIN aclroles ON aclroles.id = users.aclroles_id ";
        $join .= "LEFT JOIN aclauditorias ON users.id = aclauditorias.users_id";
        $group = 'users.' . join(',users.', $this->fields);
